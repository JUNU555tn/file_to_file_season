        ...
        if 'ids' in locals():
            ...
            for msg_id in ids:
                ...
                try:
                    ...
                    if hasattr(client, 'user_client') and client.user_client:
                        # Direct forward from saved messages to end user (no download/upload)
                        print(f"📋 Forwarding file directly from saved messages to user")
                        
                        try:
                            # Direct forward from user_client's saved messages to end user
                            sent = await client.user_client.forward_messages(
                                chat_id=user_id,
                                from_chat_id="me",
                                message_ids=msg.id
                            )
                            ...
                            if sent:
                                print(f"✅ Direct forward successful to user {user_id}")
                            else:
                                print(f"❌ Failed to forward message")
                                sent = None
                                
                        except Exception as e:
                            print(f"❌ Error in direct forward: {e}")
                            sent = None
                    else:
                        if msg.video:
